...
void GPIO_Init(GPIO_Handle_t *p_gpio_handle)
{
	...
	{
		temp = (p_gpio_handle->gpio_pin_config.gpio_pin_mode << (2 * p_gpio_handle->gpio_pin_config.gpio_pin_num));
		// clear the mode bits
		p_gpio_handle->p_gpio_x->MODER &= ~(0x3 << (2 * p_gpio_handle->gpio_pin_config.gpio_pin_num));
		// set the mode bits
		p_gpio_handle->p_gpio_x->MODER |= temp;
	}
	else
	{
		if (p_gpio_handle->gpio_pin_config.gpio_pin_mode == GPIO_MODE_IN_FE)
		{
			// enable falling trigger for input line (input line is same as pin number)
			EXTI->FTS |= (1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
			// disable rising trigger on same input line
			EXTI->RTS &= ~(1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
		}
		else if (p_gpio_handle->gpio_pin_config.gpio_pin_mode == GPIO_MODE_IN_RE)
		{
			// enable rising trigger and disable falling trigger
			EXTI->RTS |= (1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
			EXTI->FTS &= ~(1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
		}
		else if (p_gpio_handle->gpio_pin_config.gpio_pin_mode == GPIO_MODE_IN_RFT)
		{
			// enable both rising and falling trigger
			EXTI->RTS |= (1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
			EXTI->FTS |= (1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
		}
		...
		uint8_t port_code = exti_gpio_base_addr_to_code(p_gpio_handle->p_gpio_x);
		SYSCFG_PCLK_EN();
		SYSCFG->EXTICR[exti_reg_num] |= (port_code << exti_bit_offset);

		// unmask interrupts for desired input line
		EXTI->IM |= (1 << p_gpio_handle->gpio_pin_config.gpio_pin_num);
	}

	temp = 0;
	// 2. Configure speed
	temp = (p_gpio_handle->gpio_pin_config.gpio_pin_speed << (2 * p_gpio_handle->gpio_pin_config.gpio_pin_num));
	p_gpio_handle->p_gpio_x->OSPEEDR |= temp;
	temp = 0;

	// 3. Configure PU PD settings
	temp = (p_gpio_handle->gpio_pin_config.gpio_pin_pu_pd_ctrl << (2 * p_gpio_handle->gpio_pin_config.gpio_pin_num));
	p_gpio_handle->p_gpio_x->PUPDR |= temp;
	temp = 0;

	// 4. Configure output type
	temp = (p_gpio_handle->gpio_pin_config.gpio_pin_op_type << p_gpio_handle->gpio_pin_config.gpio_pin_num);
	p_gpio_handle->p_gpio_x->OTYPER |= temp;
	temp = 0;
